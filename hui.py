...
TOKEN = "8795496052:AAFMQWenw_w3MyY6n26JNleeppIrN6Q2Bk4"
ADMIN_ID = 5454985521
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        await db.commit()
    logger.info(f"База создана/проверена: {DB_PATH}")

async def get_user(user_id: int, bot: Bot):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if row is None:
            ...
            username = chat.username or "NoName"
            await db.execute(
...
def main_menu(balance: int, bonus_remaining: int = 0, last_bet: int = 100) -> InlineKeyboardMarkup:
    ...
    if bonus_remaining > 0:
        builder.row(InlineKeyboardButton(text=f"🎁 BONUS ({bonus_remaining})", callback_data="freespin"))
    else:
        builder.row(
...
async def start(message: types.Message):
    ...
    balance, _, banned, _, bonus_remaining, last_bet = await get_user(user_id, message.bot)
    if banned:
    ...
    text = (
        ...
        "Крути, покупай бонуски, лови джекпоты (иногда) 😈"
    )
...
async def cb_balance(c: CallbackQuery):
    balance, _, _, _, bonus_remaining, last_bet = await get_user(c.from_user.id, c.bot)
...
async def cb_daily(c: CallbackQuery):
    ...
    balance, last_daily, banned, _, bonus_remaining, last_bet = await get_user(user_id, c.bot)
    if banned:
...
async def buy_bonus(c: CallbackQuery):
    user_id = c.from_user.id
    balance, _, banned, _, bonus_remaining, last_bet = await get_user(user_id, c.bot)
...
async def cb_spin(c: CallbackQuery):
    ...
    balance, _, _, _, bonus_remaining, last_bet = await get_user(user_id, c.bot)
